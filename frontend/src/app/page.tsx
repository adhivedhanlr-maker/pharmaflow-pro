...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const handleNewOrder = () => {
      setStats((previous) => ({
        ...previous,
        salesToday: previous.salesToday + 1,
      }));
    };
  ...
  useEffect(() => {
    if (authLoading || !token || !role || role === "SALES_REP") {
      return;
    }
  ...
  const fetchStats = useCallback(async () => {
    ...
    const requestDefinitions = [
      {
        key: "products",
        enabled: hasRoleAccess(role, ["ADMIN", "WAREHOUSE_MANAGER", "ACCOUNTANT"]),
        request: () => fetchJsonWithTimeout<{ data?: ProductWithBatches[] }>(`${API_BASE}/inventory/products`, { headers }),
      },
      {
        key: "customers",
        enabled: hasRoleAccess(role, ["ADMIN", "BILLING_OPERATOR", "ACCOUNTANT", "SALES_REP"]),
        request: () => fetchJsonWithTimeout<{ data?: unknown[] }>(`${API_BASE}/parties/customers`, { headers }),
      },
      {
        key: "expiring",
        enabled: hasRoleAccess(role, ["ADMIN", "WAREHOUSE_MANAGER"]),
        request: () => fetchJsonWithTimeout<unknown[]>(`${API_BASE}/inventory/alerts/expiring`, { headers }),
      },
      {
        key: "lowStock",
        enabled: hasRoleAccess(role, ["ADMIN", "WAREHOUSE_MANAGER"]),
        request: () => fetchJsonWithTimeout<unknown[]>(`${API_BASE}/inventory/alerts/low-stock`, { headers }),
      },
      {
        key: "sales",
        enabled: hasRoleAccess(role, ["ADMIN", "BILLING_OPERATOR", "ACCOUNTANT", "SALES_REP"]),
        request: () => fetchJsonWithTimeout<unknown[]>(`${API_BASE}/sales/invoices`, { headers }),
      },
    ] as const;
    ...
    try {
      const settled = await Promise.all(
        requestDefinitions.map(async (definition) => {
          if (!definition.enabled) {
            return [definition.key, { ok: true, data: null, status: null }] as const;
          }

          const result = await definition.request();
          return [definition.key, result] as const;
        }),
      );

      const results = Object.fromEntries(settled) as Record<string, { ok: boolean; data: unknown; error?: string }>;
      const warnings = settled.reduce<string[]>((messages, [key, result]) => {
        if (!result.ok) {
          messages.push(`${key}: ${"error" in result ? result.error || "failed" : "failed"}`);
        }
      ...
      const totalBatches = products.reduce((accumulator, product) => accumulator + (product.batches?.length || 0), 0);
  ...
  const fetchProfileCompleteness = useCallback(async () => {
    if (!token || !role || !isAdminLikeRole(role)) {
      setProfileAlertMessage(null);
      return;
    }
    ...
    if (!profileResponse.ok && !brandingResponse.ok) {
      setProfileAlertMessage(null);
      return;
    }
